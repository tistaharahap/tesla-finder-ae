...
def configure_logfire(
    service_name: str = "tesla-finder-ae", service_version: str = "0.1.0", environment: str | None = None
) -> None:
    """
...
def get_logfire_tags() -> dict[str, Any]:
    ...
    return {"component": "tesla-finder-ae", "operation_type": "tesla_analysis"}
...
class TeslaObservabilityMixin:
    ...
    def log_url_processing_start(url: str, operation: str) -> None:
        ...
        logfire.info("🚗 Starting Tesla {operation} for URL", operation=operation, url=url, **get_logfire_tags())
    ...
    def log_batch_processing_start(urls: list[str]) -> None:
        ...
        logfire.info("📊 Starting Tesla batch processing", url_count=len(urls), urls=urls, **get_logfire_tags())

    @staticmethod
    def log_batch_processing_complete(
        total_urls: int, successful: int, failed: int, processing_time_seconds: float
    ) -> None:
...
def tesla_operation_span(operation_name: str):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            with logfire.span(f"Tesla {operation_name}", operation=operation_name, **get_logfire_tags()) as span:
                try:
