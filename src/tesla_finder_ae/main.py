...
from tesla_finder_ae.nodes import generate_consolidated_daily_tesla_digest, search_tesla_listings
...
def start_dev_server_and_open_browser():
    ...
    with logfire.span("Development Server Startup") as server_span:
        try:
            ...
            print("\n🎯 Tesla Report Ready!")
            print("   📊 View your Tesla market analysis at: http://127.0.0.1:8000")
            ...
            print("   📁 Please check that public/index.html and public/listings.json exist")
            print("   🔗 You can manually open public/index.html in your browser")
...
@app.command()
def digest(output_file: Path | None = None, custom_urls: str | None = None, html_report: bool = False):
    """
    ...
    with logfire.span(
        ...
        logfire.info(
            "✅ Consolidated digest generation completed",
        ...
        if html_report:
            with logfire.span("HTML Report Generation") as html_span:
                try:
                    ...
                    generate_tesla_html_report(consolidated_summary, html_path)
                    generate_tesla_listings_json(consolidated_summary, json_path)
                    ...
                    html_span.set_attributes(
                        {
                            ...
                            "cars_with_images": sum(
                                1 for car in consolidated_summary.all_sorted_listings if car.image_url
                            ),
                    ...
                    logfire.info(
                        "🌐 Tesla HTML+JSON report generated successfully",
                        html_path=str(html_path),
                        json_path=str(json_path),
...
@app.command()
def search(url: str, output_file: Path | None = None):
    """
    ...
    with logfire.span(
        "CLI Tesla Search Command", command="search", url=url, has_output_file=output_file is not None
    ) as cli_span:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logfire.error("💥 Tesla Finder AE CLI crashed", error_type=type(e).__name__, error_message=str(e))
        raise
